...
2) Spin up http server called `authserver.py`

# Usage

If the password = admin, you can log in, otherwise you can not

1) Start authentication test using `doveadm auth test "user@example.com" "admin"`
2) See a request come in on http server `GET /?username=user@example.com&password=admin HTTP/1.1" 200`
